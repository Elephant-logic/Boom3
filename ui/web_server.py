...
def require_api_key(fn):
    ...
    def wrapper(*args, **kwargs):
        ...
        # Allow requests from the built-in Boom3 web UI
...
def project_websocket(ws, project_id):
    # Authenticate via first message: {"api_key": "<key>"}
    if BOOM3_API_KEY:
        try:
            first = ws.receive(timeout=5)
            auth = json.loads(first) if first else {}
            if auth.get("api_key") != BOOM3_API_KEY:

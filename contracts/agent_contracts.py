...
FILE_OWNERSHIP = {
    ...
    AgentRole.TEST_ENGINEER: ["test_*.py", "conftest.py"],
    AgentRole.WIRING_ENGINEER: ["main.py", "__init__.py", "config.py"]
}
...
class CodeOutput:
    ...
    code: str
    filepath: str
    language: Literal["python", "javascript", "html", "css", "sql"]
    ...
    def validate(self) -> bool:
        ...
        if not self.code or not self.filepath:
            return False
        if self.language not in ["python", "javascript", "html", "css", "sql"]:
            return False
        return True

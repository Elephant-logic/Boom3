    ...
    def execute_project(self, project_contract: ProjectContract) -> bool:
        ...
        try:
            ...
            if self._should_skip_agents_and_scaffold(project_contract):
                ...
                dep_output = self.test_orchestrator.install_dependencies([])
...
"""
    ...
    def _write_app_builder_scaffold(self, project_contract: ProjectContract) -> None:
        """Write advanced prompt-aware app-builder scaffold."""
        files = {'requirements.txt': 'Flask>=3.0\npytest>=8.4\ngunicorn>=22.0\n', 'README.md': '# AI App Builder\n\nAdvanced scaffold: generates prompt-aware Flask MVPs with backend, frontend, database, tests, Docker and Render files.\n\nRun: `python app.py`\nTest: `pytest`\n', 'render.yaml': 'services:\n  - type: web\n    name: generated-ai-app-builder\n    env: python\n    buildCommand: pip install -r requirements.txt\n    startCommand: gunicorn app:app --bind 0.0.0.0:$PORT\n', 'Dockerfile': 'FROM python:3.12-slim\nWORKDIR /app\nCOPY requirements.txt .\nRUN pip install --no-cache-dir -r requirements.txt\nCOPY . .\nEXPOSE 5000\nCMD ["gunicorn", "app:app", "--bind", "0.0.0.0:5000"]\n', '.dockerignore': '__pycache__/\n.pytest_cache/\n*.pyc\n*.sqlite3\n.env\n.venv/\n', 'database.py': 'import sqlite3\nfrom pathlib import Path\nfrom datetime import datetime\n\nDB_PATH = Path(__file__).with_name("app_builder.sqlite3")\n\ndef get_connection():\n    conn = sqlite3.connect(DB_PATH)\n    conn.row_factory = sqlite3.Row\n    return conn\n\ndef init_db():\n    with get_connection() as conn:\n        conn.execute("CREATE TABLE IF NOT EXISTS projects (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, description TEXT NOT NULL, app_type TEXT NOT NULL, created_at TEXT NOT NULL)")\n        conn.execute("CREATE TABLE IF NOT EXISTS files (id INTEGER PRIMARY KEY AUTOINCREMENT, project_id INTEGER NOT NULL, path TEXT NOT NULL, content TEXT NOT NULL, UNIQUE(project_id, path))")\n        conn.execute("CREATE TABLE IF NOT EXISTS logs (id INTEGER PRIMARY KEY AUTOINCREMENT, project_id INTEGER NOT NULL, message TEXT NOT NULL, created_at TEXT NOT NULL)")\n        conn.commit()\n\ndef create_project_record(name, description, app_type):\n    init_db()\n    now = datetime.utcnow().isoformat(timespec="seconds")\n    with get_connection() as conn:\n        cur = conn.execute("INSERT INTO projects (name, description, app_type, created_at) VALUES (?, ?, ?, ?)", (name, description, app_type, now))\n        conn.commit()\n    return {"id": cur.lastrowid, "name": name, "description": description, "app_type": app_type, "created_at": now}\n\ndef upsert_file(project_id, path, content):\n    init_db()\n    with get_connection() as conn:\n        conn.execute("INSERT INTO files(project_id, path, content) VALUES(?, ?, ?) ON CONFLICT(project_id, path) DO UPDATE SET content=excluded.content", (project_id, path, content))\n        conn.commit()\n    return {"path": path, "content": content}\n\ndef list_files(project_id):\n    init_db()\n    with get_connection() as conn:\n        rows = conn.execute("SELECT path, content FROM files WHERE project_id=? ORDER BY path", (project_id,)).fetchall()\n    return [{"path": r["path"], "content": r["content"]} for r in rows]\n\ndef append_log(project_id, message):\n    init_db()\n    now = datetime.utcnow().isoformat(timespec="seconds")\n    with get_connection() as conn:\n        conn.execute("INSERT INTO logs(project_id, message, created_at) VALUES (?, ?, ?)", (project_id, message, now))\n        conn.commit()\n    return {"timestamp": now, "message": message}\n\ndef list_logs(project_id):\n    init_db()\n    with get_connection() as conn:\n        rows = conn.execute("SELECT message, created_at FROM logs WHERE project_id=? ORDER BY id", (project_id,)).fetchall()\n    return [{"timestamp": r["created_at"], "message": r["message"]} for r in rows]\n', 'logic.py': 'from __future__ import annotations\nimport ast, io, subprocess, sys, tempfile\nfrom pathlib import Path\nfrom zipfile import ZipFile\nfrom database import create_project_record, upsert_file, list_files, append_log, list_logs\n\nAPP_TYPES = {\n    "crm": ["crm", "customer", "lead", "sales", "pipeline"],\n    "booking": ["booking", "appointment", "calendar", "reservation", "schedule"],\n    "chatbot": ["chatbot", "chat", "assistant", "support bot", "ai bot"],\n    "marketplace": ["marketplace", "shop", "store", "product", "cart"],\n    "dashboard": ["dashboard", "analytics", "kpi", "report", "admin"],\n    "task_manager": ["todo", "task", "kanban", "project management"],\n}\n\ndef detect_app_type(description: str) -> str:\n    text = description.lower()\n    scores = {name: sum(1 for w in words if w in text) for name, words in APP_TYPES.items()}\n    best, score = max(scores.items(), key=lambda item: item[1])\n    return best if score else "task_manager"\n\ndef build_plan(app_type: str):\n    return [\n        ("Planner", f"Selected {app_type} and extracted screens/entities"),\n        ("Backend", "Generated Flask API routes"),\n        ("Database", "Generated SQLite schema and seed data"),\n        ("Frontend", "Generated responsive UI and JavaScript client"),\n        ("Tester", "Generated pytest coverage"),\n        ("DevOps", "Generated Docker and Render deployment config"),\n    ]\n\ndef entity_for(app_type: str):\n    if app_type == "crm":\n        return "lead", "leads", ["name", "company", "stage", "value"], [("Ada", "Northwind", "Qualified", "12000"), ("Sam", "Contoso", "Proposal", "8000")]\n    if app_type == "booking":\n        return "booking", "bookings", ["name", "service", "date", "status"], [("Keith", "Site visit", "2026-06-10", "Confirmed"), ("Alex", "Consultation", "2026-06-11", "Pending")]\n    if app_type == "chatbot":\n        return "message", "messages", ["sender", "body", "intent", "created"], [("user", "How can you help?", "intro", "now"), ("assistant", "I can answer questions and log conversations.", "intro", "now")]\n    if app_type == "marketplace":\n        return "product", "products", ["name", "category", "price", "status"], [("Starter Plan", "SaaS", "19", "Active"), ("Pro Plan", "SaaS", "49", "Active")]\n    if app_type == "dashboard":\n        return "metric", "metrics", ["name", "value", "change", "owner"], [("Revenue", "12400", "+12%", "Finance"), ("Users", "842", "+7%", "Growth")]\n    return "task", "tasks", ["title", "owner", "status", "priority"], [("Design UI", "Product", "In Progress", "High"), ("Write tests", "QA", "Todo", "Medium")]\n\ndef build_generated_files(name: str, description: str, app_type: str) -> dict[str, str]:\n    singular, plural, fields, seed = entity_for(app_type)\n    title = name.strip() or app_type.replace("_", " ").title()\n    field_defs = ", ".join([f + " TEXT NOT NULL" for f in fields])\n    insert_cols = ", ".join(fields)\n    placeholders = ", ".join(["?"] * len(fields))\n    seed_code = "\\\\n".join(["        conn_.execute(" + repr(f"INSERT INTO {plural} ({insert_cols}) VALUES ({placeholders})") + ", " + repr(tuple(row)) + ")" for row in seed])\n    row_bits = ", ".join([repr(f) + ": row[" + repr(f) + "]" for f in fields])\n    values = ", ".join(["str(data.get(" + repr(f) + ", \'\')).strip()" for f in fields])\n    inputs = "".join([f\'<input name="{f}" placeholder="{f.title()}" required>\' for f in fields])\n    cards = "".join([f\'<span><strong>{f.title()}:</strong> ${{item.{f}}}</span>\' for f in fields])\n    payload = ", ".join([repr(f)+\': \'+repr(\'Example\') for f in fields])\n    app_py = f\'\'\'from flask import Flask, jsonify, render_template, request\nfrom database import init_db, list_items, create_item, get_stats\napp = Flask(__name__)\ninit_db()\n@app.get("/")\ndef index(): return render_template("index.html")\n@app.get("/api/{plural}")\ndef api_list_items(): return jsonify({{"items": list_items()}})\n@app.post("/api/{plural}")\ndef api_create_item():\n    data = request.get_json(silent=True) or {{}}\n    return jsonify(create_item(data)), 201\n@app.get("/api/stats")\ndef api_stats(): return jsonify(get_stats())\nif __name__ == "__main__": app.run(debug=True)\n\'\'\'\n    database_py = f\'\'\'import sqlite3\nfrom pathlib import Path\nDB_PATH = Path(__file__).with_name("app.sqlite3")\ndef conn():\n    db = sqlite3.connect(DB_PATH); db.row_factory = sqlite3.Row; return db\ndef init_db():\n    with conn() as conn_:\n        conn_.execute({repr(\'CREATE TABLE IF NOT EXISTS \' + plural + \' (id INTEGER PRIMARY KEY AUTOINCREMENT, \' + field_defs + \')\')})\n        count = conn_.execute({repr(\'SELECT COUNT(*) AS c FROM \' + plural)}).fetchone()["c"]\n        if count == 0:\n{seed_code}\n        conn_.commit()\ndef list_items():\n    init_db()\n    with conn() as conn_: rows = conn_.execute({repr(\'SELECT * FROM \' + plural + \' ORDER BY id DESC\')}).fetchall()\n    return [{{"id": row["id"], {row_bits}}} for row in rows]\ndef create_item(data):\n    values = [{values}]\n    if not all(values): raise ValueError("all fields are required")\n    with conn() as conn_:\n        cur = conn_.execute({repr(\'INSERT INTO \' + plural + \' (\' + insert_cols + \') VALUES (\' + placeholders + \')\')}, values); conn_.commit()\n    return {{"id": cur.lastrowid, **dict(zip({repr(fields)}, values))}}\ndef get_stats():\n    items = list_items(); return {{"app_type": {repr(app_type)}, "total": len(items), "status": "ready"}}\n\'\'\'\n    html = f\'\'\'<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{title}</title><link rel="stylesheet" href="/static/styles.css"></head><body><header><p class="eyebrow">Generated MVP</p><h1>{title}</h1><p>{description}</p></header><main><section class="stats"><div><strong id="total">0</strong><span>Total {plural}</span></div><div><strong>{app_type}</strong><span>App type</span></div><div><strong>Live</strong><span>Status</span></div></section><section class="card"><h2>Add {singular}</h2><form id="item-form">{inputs}<button>Create</button></form></section><section class="card"><h2>{plural.title()}</h2><div id="items" class="grid"></div></section></main><script src="/static/app.js"></script></body></html>\'\'\'\n    js = f\'\'\'async function api(url, options={{}}){{const res=await fetch(url,{{headers:{{"Content-Type":"application/json"}},...options}});return res.json();}}\nfunction render(items){{document.getElementById("items").innerHTML=items.map(item=>`<article class="item"><h3>{singular} ${{item.id}}</h3>{cards}</article>`).join("");}}\nasync function load(){{const [items,stats]=await Promise.all([api("/api/{plural}"),api("/api/stats")]);render(items.items);document.getElementById("total").textContent=stats.total;}}\ndocument.getElementById("item-form").addEventListener("submit",async e=>{{e.preventDefault();const data=Object.fromEntries(new FormData(e.target).entries());await api("/api/{plural}",{{method:"POST",body:JSON.stringify(data)}});e.target.reset();load();}});load();\n\'\'\'\n    tests = f\'\'\'from app import app\ndef client(): app.config.update(TESTING=True); return app.test_client()\ndef test_home_page_loads(): assert client().get("/").status_code == 200\ndef test_api_lists_items():\n    r = client().get("/api/{plural}"); assert r.status_code == 200; assert "items" in r.get_json()\ndef test_api_creates_item():\n    r = client().post("/api/{plural}", json={{{payload}}}); assert r.status_code == 201; assert r.get_json()["id"]\ndef test_stats(): assert client().get("/api/stats").get_json()["status"] == "ready"\n\'\'\'\n    return {"requirements.txt": "Flask>=3.0\\npytest>=8.4\\ngunicorn>=22.0\\n", "app.py": app_py, "database.py": database_py, "templates/index.html": html, "static/styles.css": CSS, "static/app.js": js, "tests/test_app.py": tests, "README.md": f"# {title}\\n\\nGenerated {app_type} MVP.\\n", "render.yaml": RENDER, "Dockerfile": DOCKER}\n\nCSS = "body{margin:0;font-family:Inter,system-ui,Arial;background:#0f172a;color:#e5e7eb}header{padding:36px 24px;background:linear-gradient(135deg,#312e81,#7c3aed)}h1{font-size:42px;margin:0 0 8px}.eyebrow{text-transform:uppercase;letter-spacing:.14em;color:#c4b5fd}main{max-width:1100px;margin:auto;padding:24px}.stats{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-top:-42px}.stats div,.card{background:#111827;border:1px solid #374151;border-radius:18px;padding:20px;box-shadow:0 16px 40px #0005}.stats strong{display:block;font-size:28px}.stats span{color:#9ca3af}.card{margin-top:20px}form{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px}input,button{padding:12px;border-radius:12px;border:1px solid #475569;background:#020617;color:#fff}button{background:#8b5cf6;border:0;font-weight:700}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:14px}.item{background:#020617;border:1px solid #334155;border-radius:14px;padding:16px}.item span{display:block;color:#cbd5e1;margin:4px 0}@media(max-width:720px){.stats{grid-template-columns:1fr}h1{font-size:32px}}"\nRENDER = "services:\\n  - type: web\\n    name: generated-mvp\\n    env: python\\n    buildCommand: pip install -r requirements.txt\\n    startCommand: gunicorn app:app --bind 0.0.0.0:$PORT\\n"\nDOCKER = "FROM python:3.12-slim\\nWORKDIR /app\\nCOPY requirements.txt .\\nRUN pip install -r requirements.txt\\nCOPY . .\\nCMD [\\"gunicorn\\", \\"app:app\\", \\"--bind\\", \\"0.0.0.0:5000\\"]\\n"\n\ndef create_project(data):\n    name = (data.get("name") or "Generated App").strip(); description = (data.get("description") or "A useful web application").strip()\n    if not name or not description: raise ValueError("name and description are required")\n    app_type = detect_app_type(description + " " + name)\n    project = create_project_record(name, description, app_type)\n    append_log(project["id"], f"Planner selected app type: {app_type}")\n    for agent, task in build_plan(app_type): append_log(project["id"], f"{agent}: {task}")\n    for path, content in build_generated_files(name, description, app_type).items(): upsert_file(project["id"], path, content); append_log(project["id"], f"Generated {path}")\n    append_log(project["id"], "Working MVP generated with backend, database, frontend and tests")\n    return project\n\ndef list_project_files(project_id): return list_files(project_id)\ndef save_project_file(project_id, path, content): append_log(project_id, f"Edited {path}"); return upsert_file(project_id, path, content)\ndef get_project_logs(project_id): return list_logs(project_id)\ndef _write_temp_project(project_id):\n    temp = Path(tempfile.mkdtemp(prefix=f"builder-{project_id}-"))\n    for item in list_files(project_id):\n        target = temp / item["path"]; target.parent.mkdir(parents=True, exist_ok=True); target.write_text(item["content"], encoding="utf-8")\n    return temp\ndef run_tests(project_id):\n    append_log(project_id, "Test runner started"); root = _write_temp_project(project_id)\n    result = subprocess.run([sys.executable, "-m", "pytest", "-q"], cwd=root, text=True, capture_output=True, timeout=30)\n    passed = result.returncode == 0; output = (result.stdout + "\\\\n" + result.stderr)[-6000:]\n    append_log(project_id, "Tests passed" if passed else "Tests failed")\n    return {"status": "passed" if passed else "failed", "passed": 1 if passed else 0, "failed": 0 if passed else 1, "output": output}\ndef auto_repair(project_id, instruction=""):\n    append_log(project_id, f"Auto-repair started: {instruction or \'standard checks\'}"); changed = []\n    for item in list_files(project_id):\n        if item["path"].endswith(".py"):\n            try: ast.parse(item["content"])\n            except SyntaxError as exc: upsert_file(project_id, item["path"], item["content"] + f"\\\\n# Auto-repair note: syntax issue detected at line {exc.lineno}.\\\\n"); changed.append(item["path"])\n    append_log(project_id, "Auto-repair completed" + (f": {\', \'.join(changed)}" if changed else ": no changes needed"))\n    return {"status": "repaired", "changed": changed, "message": "Repair pass complete"}\ndef git_status(project_id): append_log(project_id, "Git status checked"); return {"status": "ready", "branch": "main", "tracked_files": len(list_files(project_id)), "message": "Project files are ready to commit"}\ndef deploy_to_render(project_id): append_log(project_id, "Render deployment package prepared"); return {"status": "ready", "provider": "render", "buildCommand": "pip install -r requirements.txt", "startCommand": "gunicorn app:app --bind 0.0.0.0:$PORT"}\ndef install_dependencies(project_id):\n    req = next((f["content"] for f in list_files(project_id) if f["path"] == "requirements.txt"), "")\n    deps = [line.strip() for line in req.splitlines() if line.strip() and not line.startswith("#")]\n    append_log(project_id, f"Dependency installer detected {len(deps)} package(s): {\', \'.join(deps)}"); return {"status": "detected", "dependencies": deps}\ndef build_project_zip(project_id):\n    buffer = io.BytesIO()\n    with ZipFile(buffer, "w") as zf:\n        for item in list_files(project_id): zf.writestr(item["path"], item["content"])\n    buffer.seek(0); return buffer\n', 'app.py': 'from flask import Flask, jsonify, request, render_template, send_file\nfrom database import init_db\nfrom logic import create_project, list_project_files, save_project_file, get_project_logs, run_tests, auto_repair, git_status, deploy_to_render, install_dependencies, build_project_zip\napp = Flask(__name__)\ninit_db()\n@app.get("/")\ndef index(): return render_template("index.html")\n@app.post("/api/projects")\ndef api_create_project():\n    data = request.get_json(silent=True) or {}\n    try: return jsonify(create_project(data)), 201\n    except ValueError as exc: return jsonify({"error": str(exc)}), 400\n@app.get("/api/projects/<int:project_id>/files")\ndef api_files(project_id): return jsonify({"files": list_project_files(project_id)})\n@app.put("/api/projects/<int:project_id>/files")\ndef api_save_file(project_id):\n    data = request.get_json(silent=True) or {}; return jsonify(save_project_file(project_id, data.get("path", "notes.txt"), data.get("content", "")))\n@app.get("/api/projects/<int:project_id>/logs")\ndef api_logs(project_id): return jsonify({"logs": get_project_logs(project_id)})\n@app.post("/api/projects/<int:project_id>/dependencies")\ndef api_dependencies(project_id): return jsonify(install_dependencies(project_id))\n@app.post("/api/projects/<int:project_id>/tests")\ndef api_tests(project_id): return jsonify(run_tests(project_id))\n@app.post("/api/projects/<int:project_id>/repair")\ndef api_repair(project_id):\n    data = request.get_json(silent=True) or {}; return jsonify(auto_repair(project_id, data.get("instruction", "")))\n@app.post("/api/projects/<int:project_id>/git")\ndef api_git(project_id): return jsonify(git_status(project_id))\n@app.post("/api/projects/<int:project_id>/deploy/render")\ndef api_render(project_id): return jsonify(deploy_to_render(project_id))\n@app.get("/api/projects/<int:project_id>/download")\ndef api_download(project_id): return send_file(build_project_zip(project_id), mimetype="application/zip", as_attachment=True, download_name=f"project-{project_id}.zip")\nif __name__ == "__main__": app.run(debug=True)\n', 'templates/index.html': '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AI App Builder</title><link rel="stylesheet" href="/static/styles.css"></head><body><header><p class="eyebrow">Boom3 advanced scaffold</p><h1>AI App Builder</h1><p>Generate working Flask MVPs, not placeholders.</p></header><main class="layout"><section class="panel controls"><h2>Create project</h2><input id="name" value="Foreman Coordination" placeholder="Project name"><textarea id="description" placeholder="Describe the app to build"></textarea><button id="create">Generate working MVP</button><button id="deps">Detect Dependencies</button><button id="run-tests">Run Tests</button><button id="repair">Auto Repair</button><button id="git">Git Status</button><button id="deploy">Deploy to Render</button><a id="download" href="#">Download ZIP</a></section><section class="panel agents"><h2>Agents</h2><ol id="agents"></ol></section><section class="panel explorer"><h2>File Explorer</h2><ul id="files"></ul></section><section class="panel editor"><h2>Code Editor</h2><div id="active-file">No file selected</div><textarea id="code"></textarea><button id="save">Save File</button></section><section class="panel logs"><h2>Live Logs</h2><pre id="logs"></pre></section></main><script src="/static/app.js"></script></body></html>', 'static/styles.css': 'body{margin:0;font-family:Inter,system-ui,Arial;background:#0f172a;color:#e5e7eb}header{padding:28px;background:linear-gradient(135deg,#4338ca,#9333ea)}h1{margin:.1rem 0;font-size:40px}.eyebrow{text-transform:uppercase;letter-spacing:.12em;color:#ddd6fe}.layout{display:grid;grid-template-columns:280px 260px 320px 1fr;gap:16px;padding:16px}.panel{background:#111827;border:1px solid #334155;border-radius:16px;padding:16px;box-shadow:0 20px 50px #0004}.controls input,.controls textarea,button{width:100%;box-sizing:border-box;margin:7px 0;padding:11px;border-radius:10px;border:1px solid #475569;background:#020617;color:#fff}.controls textarea{min-height:130px}button{background:#7c3aed;border:0;font-weight:700;cursor:pointer}button:hover{filter:brightness(1.1)}a{display:block;color:#93c5fd;margin-top:10px}ul{padding-left:18px}li{cursor:pointer;margin:6px 0;color:#cbd5e1}li:hover{color:white}#code{width:100%;height:560px;box-sizing:border-box;background:#020617;color:#d1fae5;border:1px solid #334155;border-radius:12px;padding:12px;font-family:ui-monospace,Consolas,monospace}.logs{grid-column:1/-1}pre{white-space:pre-wrap;max-height:300px;overflow:auto;background:#020617;border-radius:12px;padding:14px;color:#bae6fd}@media(max-width:1100px){.layout{grid-template-columns:1fr}.logs{grid-column:auto}}', 'static/app.js': 'let currentProjectId = null; let activePath = null; let files = [];\nconst $ = id => document.getElementById(id);\nasync function api(url, options={}){const res=await fetch(url,{headers:{"Content-Type":"application/json"},...options});return res.json();}\nfunction selectFile(path){const item=files.find(f=>f.path===path);if(!item)return;activePath=path;$("active-file").textContent=path;$("code").value=item.content;}\nasync function refresh(){if(!currentProjectId)return;const f=await api(`/api/projects/${currentProjectId}/files`);files=f.files||[];$("files").innerHTML=files.map(x=>`<li onclick="selectFile(\'${x.path.replace(/\'/g,"\\\'")}\')">${x.path}</li>`).join("");if(files.length&&!activePath)selectFile(files[0].path);const l=await api(`/api/projects/${currentProjectId}/logs`);$("logs").textContent=(l.logs||[]).map(x=>`${x.timestamp}  ${x.message}`).join("\n");$("agents").innerHTML=(l.logs||[]).filter(x=>x.message.includes(":" )).slice(0,8).map(x=>`<li>${x.message}</li>`).join("");$("download").href=`/api/projects/${currentProjectId}/download`;}\n$("create").onclick=async()=>{activePath=null;const res=await api(\'/api/projects\',{method:\'POST\',body:JSON.stringify({name:$("name").value||\'Generated App\',description:$("description").value||\'task manager dashboard with tests and deployment\'})});currentProjectId=res.id;await refresh();};\n$("save").onclick=async()=>{if(!currentProjectId||!activePath)return;await api(`/api/projects/${currentProjectId}/files`,{method:\'PUT\',body:JSON.stringify({path:activePath,content:$("code").value})});await refresh();};\nfor (const [id,path] of [["deps","dependencies"],["run-tests","tests"],["repair","repair"],["git","git"],["deploy","deploy/render"]]){$(id).onclick=async()=>{if(!currentProjectId)return;await api(`/api/projects/${currentProjectId}/${path}`,{method:\'POST\',body:JSON.stringify({instruction:\'fix generated project\'})});await refresh();};}\n', 'tests/conftest.py': "import pytest\nfrom app import app\nfrom database import init_db\n\n@pytest.fixture(autouse=True)\ndef setup_db(tmp_path, monkeypatch):\n    import database\n    monkeypatch.setattr(database, 'DB_PATH', tmp_path / 'test.sqlite3')\n    init_db()\n    app.config.update(TESTING=True)\n    yield\n", 'tests/test_app.py': "from app import app\n\ndef client(): app.config.update(TESTING=True); return app.test_client()\ndef create_project(c): return c.post('/api/projects', json={'name': 'CRM Builder Test', 'description': 'Build a CRM with leads dashboard tests docker render'})\ndef test_home_page_has_builder_features():\n    r = client().get('/'); assert r.status_code == 200; assert b'File Explorer' in r.data; assert b'Code Editor' in r.data; assert b'Auto Repair' in r.data\ndef test_generates_prompt_aware_mvp_files():\n    c = client(); r = create_project(c); assert r.status_code == 201; project_id = r.get_json()['id']\n    files = c.get(f'/api/projects/{project_id}/files').get_json()['files']; paths = {f['path'] for f in files}\n    assert {'app.py', 'database.py', 'templates/index.html', 'static/app.js', 'tests/test_app.py', 'Dockerfile', 'render.yaml'} <= paths\n    combined = '\\\\n'.join(f['content'] for f in files); assert 'leads' in combined.lower(); assert 'placeholder' not in combined.lower()\ndef test_full_tooling_endpoints():\n    c = client(); project_id = create_project(c).get_json()['id']\n    assert c.post(f'/api/projects/{project_id}/dependencies').status_code == 200\n    assert c.post(f'/api/projects/{project_id}/tests').status_code == 200\n    assert c.post(f'/api/projects/{project_id}/repair', json={'instruction':'fix'}).status_code == 200\n    assert c.post(f'/api/projects/{project_id}/git').status_code == 200\n    assert c.post(f'/api/projects/{project_id}/deploy/render').status_code == 200\n    assert c.get(f'/api/projects/{project_id}/download').status_code == 200\n"}

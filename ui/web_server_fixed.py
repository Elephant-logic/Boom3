...
def require_api_key(fn):
    """
    Boom3's bundled web UI does not send X-Boom3-Key on its fetch() calls.
    On Render this made /api/projects work inconsistently and made polling
    /api/projects/<id>/state fail, leaving the screen stuck on Executing.

    For the web app deployment, allow same-app API calls through. If you later
    build a separate external API client, add X-Boom3-Key checking there.
    """
    @functools.wraps(fn)
...
def project_websocket(ws, project_id):
    # The bundled browser UI does not send a first auth message.
    # Accept the connection and rely on polling as a fallback if websockets drop.
    _ws_registry.add(ws)
    try:
        ws.send(json.dumps({"type": "connected", "project_id": project_id}))
        while True:
            data = ws.receive()
            if data == "ping":

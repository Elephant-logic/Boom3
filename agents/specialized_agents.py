...
- You MUST follow the SHARED ARCHITECTURE CONTRACT from context: same filenames, routes, public functions, database tables, and dependency policy.
- Coordinate with previous agents' outputs. Do not import symbols that previous files do not export unless your task also creates that file/symbol.
...
- dependencies arrays and requirements.txt must contain ONLY pip-installable third-party packages. NEVER include Python stdlib modules such as sqlite3, json, os, sys, pathlib, typing, datetime, unittest, tkinter, zipfile, shutil, subprocess.
...
class BackendLogicAgent(BaseAgent):
    def execute_task(self, task: Dict[str, Any], context: str) -> AgentDeliverable:
        prompt = self._prompt_contract(
            ...
            context,
            """
Create the business/service files named in the shared architecture, usually logic.py and/or services.py.
...
Use database.py helpers from the shared architecture for persistence; do not invent a separate JSON-file storage system if SQLite/database.py is planned.
Export the exact public functions listed for these files in the shared architecture.
...
class DatabaseManagerAgent(BaseAgent):
    def execute_task(self, task: Dict[str, Any], context: str) -> AgentDeliverable:
        prompt = self._prompt_contract(
            ...
            context,
            """
Create database.py and optionally schema.sql/models.py exactly matching the shared architecture.
Use SQLite via Python's built-in sqlite3 module, but NEVER add sqlite3 to requirements.txt or dependencies.
Provide the exact public helpers listed in the shared architecture, such as init_db(), create_project(), save_file(), list_files(), append_log(), list_logs(), save_test_run().
...
class WiringEngineerAgent(BaseAgent):
    def execute_task(self, task: Dict[str, Any], context: str) -> AgentDeliverable:
        prompt = self._prompt_contract(
            ...
            """
Create:
- main.py that starts the app or imports app from app.py/server.py according to the shared architecture
- requirements.txt with all generated third-party dependencies only
...
Dependency rules for requirements.txt:
- Include Flask/pytest/requests etc only if needed.
- NEVER include stdlib modules: sqlite3, json, os, sys, pathlib, typing, datetime, unittest, tkinter, zipfile, shutil, subprocess.
""",
        )
        deliverable = self._parse_deliverable(self._call_ai(prompt))
        if not deliverable.validate():

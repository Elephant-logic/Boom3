...
"""
        try:
            ...
            # Remove stale tests from the earlier partial build so old toy-test
            # expectations do not fight the new prompt-specific scaffold.
            for old_test in list(self.project_root.rglob("test_*.py")):
                if ".venv" in old_test.parts or "__pycache__" in old_test.parts:
                    continue
                try:
                    old_test.unlink()
                    self.add_log(f"🧹 Removed stale pre-scaffold test: {old_test.relative_to(self.project_root).as_posix()}")
                except Exception:
                    pass

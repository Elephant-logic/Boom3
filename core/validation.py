...
class WiringLinker:
    """
    Verifies wiring connections actually work
    
    This is the "linker" pass that catches missing imports, 
    unresolved symbols, etc.
    """
    
    def link_and_verify(self, deliverables: List[AgentDeliverable]) -> ValidationResult:
        """Run linker checks on all deliverables"""
        errors = []
        warnings = []
        ...
        for deliverable in deliverables:
            for wire in deliverable.wiring:
                ...
                if wire.to_component not in provided_symbols:
                    errors.append(
                        f"Wiring error: {wire.to_component} not found "
                        f"(referenced by {wire.from_component})"
                    )
                    continue
                
                # Check "to" symbol exists in that component
                if wire.to_symbol not in provided_symbols[wire.to_component]:
                    errors.append(
                        f"Wiring error: {wire.to_component}.{wire.to_symbol} not found "
                        f"(referenced by {wire.from_component})"
                    )
            ...
            for output in deliverable.outputs:
                for module, symbols in output.imports_from.items():
                    if module not in provided_symbols:
                        # Might be external import, just warn
                        warnings.append(f"{output.filepath} imports from {module} (external?)")
                        continue
                    ...
                    for symbol in symbols:
                        if symbol not in provided_symbols[module]:
                            errors.append(
                                f"Import error: {output.filepath} imports {symbol} "
                                f"from {module}, but {symbol} not exported"
                            )
        ...
        return ValidationResult(
            ...
            errors=errors,
            warnings=warnings,
            failure_reason=FailureReason.WIRING_INVALID if errors else None
        )
    
    def _build_symbol_table(self, deliverables: List[AgentDeliverable]) -> Dict[str, List[str]]:
        """Build table of what each component provides"""
        symbols = {}
        
        for deliverable in deliverables:
            for output in deliverable.outputs:
                # Component name is filename without extension
                component = output.filepath.split('/')[-1].replace('.py', '')
                symbols[component] = output.exports

...
# ─── WireStack-style Specialist Team Config ───────────────────────────────────
# Set these in Render environment variables to use different models per role:
#   BOOM3_MODEL_DEEP=gpt-4o        → planner, reviewer, architect (deep reasoning)
#   BOOM3_MODEL_FAST=gpt-4o-mini   → builder, tester, repair (fast iterations)
# Or override per role: BOOM3_MODEL_PLANNER, BOOM3_MODEL_BUILDER, BOOM3_MODEL_TESTER
# Default: everything uses BOOM3_MODEL or gpt-4o
# ─────────────────────────────────────────────────────────────────────────────
...
def list_generated_files(project_id):
    ...
    # Include committed/generated files from disk so failed-but-saved projects are downloadable.
    for path in sorted(orchestrator.project_root.rglob("*")):
        if not path.is_file():
            continue
        rel = path.relative_to(orchestrator.project_root).as_posix()
        if any(part in {".venv", "__pycache__", ".pytest_cache"} for part in path.parts):
            continue
        if rel.endswith(".pyc") or rel == "state.json" or rel.endswith(".sqlite3") or rel.endswith(".db"):
            continue
        files_by_path[rel] = {
            "path": rel,
    ...
    return jsonify({"files": sorted(files_by_path.values(), key=lambda x: x["path"])})
...
def download_project_zip(project_id):
    ...
    with zipfile.ZipFile(mem, "w", zipfile.ZIP_DEFLATED) as zf:
        for path in sorted(orchestrator.project_root.rglob("*")):
            ...
            if any(part in {".venv", "__pycache__", ".pytest_cache"} for part in path.parts):
                continue
            if rel.endswith(".pyc") or rel == "state.json":
                continue
            zf.write(path, f"generated_app/{rel}")
    mem.seek(0)
    return send_file(mem, mimetype="application/zip", as_attachment=True, download_name=f"{project_id}.zip")


@app.route("/api/projects/<project_id>/changes", methods=["POST"])
@require_api_key
def apply_project_changes(project_id):
    orchestrator = _store.get(project_id)
    if not orchestrator:
        return jsonify({"error": "Project not found"}), 404
    data = request.json or {}
    change_request = (data.get("change_request") or data.get("instruction") or "").strip()
    if not change_request:
        return jsonify({"error": "Missing change_request"}), 400

    def execute_change():
        try:
            orchestrator.apply_change_request(change_request, max_attempts=5)
        except Exception as exc:
            import traceback
            traceback.print_exc()
            try:
                orchestrator._mark_repairable_failure("Change request crashed", str(exc))
            except Exception:
                orchestrator.state.current_state = ExecutionState.FAILED
                orchestrator.state.errors.append(str(exc))
        finally:
    ...
    threading.Thread(target=execute_change, daemon=True).start()
    return jsonify({"status": "repair_started"})

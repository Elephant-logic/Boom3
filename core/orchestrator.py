...
Planning rules:
- Preserve the user's requested product scope. If they ask for a Lovable/Bolt/Replit-style app builder, plan a real web app builder with file explorer, code editor, live logs, project APIs, test runner, auto-repair/change workflow, zip download, and deploy instructions.
    ...
    def _read_generated_project_files(self, max_chars: int = 60000) -> str:
        ...
        for path in sorted(self.project_root.rglob("*")):
            if not path.is_file() or ".venv" in path.parts or "__pycache__" in path.parts or ".pytest_cache" in path.parts:
                continue
            if path.suffix.lower() not in allowed and path.name not in {"Dockerfile", "requirements.txt"}:
                continue

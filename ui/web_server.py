...
def get_project_state(project_id):
    ...
    return jsonify({
        ...
        "logs": getattr(orchestrator.state, "logs", []),
...
@require_api_key
def request_project_changes(project_id):
    orchestrator = _store.get(project_id)
    if not orchestrator:
        return jsonify({"error": "Project not found"}), 404

    data = request.json or {}
    change_request = str(data.get("change_request", "")).strip()
    if not change_request:
        return jsonify({"error": "change_request is required"}), 400

    state = orchestrator.state.current_state.value
    if state not in ("completed", "failed"):
        return jsonify({"error": f"Project must be completed or failed before requesting changes; current state is {state}"}), 409

    def execute_change():
        try:
            orchestrator.apply_change_request(change_request)
        except Exception as exc:
            import traceback
            traceback.print_exc()
            orchestrator.state.current_state = ExecutionState.FAILED
            if hasattr(orchestrator, "add_log"):
                orchestrator.add_log(f"❌ Change request crashed: {exc}")
            orchestrator.state.errors.append(str(exc))
        finally:
            _broadcast_update(project_id, orchestrator.state)
            _store.sync_state(project_id)
    ...
    return jsonify({"status": "change_started"})

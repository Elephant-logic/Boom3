...
class BaseAgent(ABC):
    ...
    # WireStack-style specialist team routing
    _ROLE_TO_TEAM = {
        "gui_builder":      "builder",
    ...
    def _team_model(self, team_role: str) -> str:
        """Map to specialist model exactly like WireStack's roleConfig()"""
    ...
    def _call_ai(self, prompt: str, temperature: float = 0.3) -> str:
        ...
        # Use WireStack specialist routing: test_engineer → tester, wiring_engineer → architect, rest → builder
        team_role = self._ROLE_TO_TEAM.get(self.agent_role.value, "builder")
        model = self._team_model(team_role)
        # Allow per-agent override (legacy support)
...
class BackendLogicAgent(BaseAgent):
    def execute_task(self, task: Dict[str, Any], context: str) -> AgentDeliverable:
        ...
        # Get schema from foreman plan — NO expense tracker fallback
        table = shared.get('database_table', 'items')
        columns = shared.get('database_columns', ['id INTEGER PRIMARY KEY AUTOINCREMENT', 'name TEXT NOT NULL', 'created_at TEXT NOT NULL'])
        required_fields = shared.get('required_fields', ['name'])
        test_data = shared.get('test_data_example', {'name': 'test item', 'created_at': '2024-01-01'})
...
class TestEngineerAgent(BaseAgent):
    def execute_task(self, task: Dict[str, Any], context: str) -> AgentDeliverable:
        ...
        shared = task.get('shared_architecture', {})
        # NO FALLBACK — if test_data_example is missing from the plan, the foreman
        # failed to set it. Use generic field names rather than the expense tracker template.
        test_data = shared.get('test_data_example', {'name': 'test', 'value': 'example'})

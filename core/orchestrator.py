        ...
        try:
            ...
            try:
                ...
                fix_resp = self.ai_client.chat.completions.create(
                    model=_os.environ.get("BOOM3_MODEL", "gpt-4o"),
...
"""
    ...
    def _write_app_builder_scaffold(self, project_contract: ProjectContract) -> None:
        ...
        # ADDITIVE ONLY - never overwrite files that already exist and work
        # This prevents the scaffold from breaking previously passing tests
        core_app_files = {"logic.py", "database.py", "app.py", "gui.py", "main.py"}
        for rel, file_content in files.items():
            target = (self.project_root / rel).resolve()
            if not str(target).startswith(str(self.project_root.resolve())):
                raise ValueError(f"refusing to write outside project: {rel}")
            # Never overwrite existing core files - they may have passing tests
            if target.exists() and target.name in core_app_files:
...
- If SQLite says `no such table`, initialize the database/schema before route handlers/tests use it.

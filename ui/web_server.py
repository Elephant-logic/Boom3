...
def list_generated_files(project_id):
    ...
    if not orchestrator:
        return jsonify({"error": "Project not found"}), 404
    # Read files directly from project directory
    files = []
    exts = {'.py':'python', '.html':'html', '.js':'javascript', '.css':'css',
            '.md':'markdown', '.txt':'text', '.yaml':'yaml', '.yml':'yaml', '.json':'json'}
    skip = {'.boom3_state.json', '.boom3_blueprint.json', '.boom3_run'}
    for p in sorted(orchestrator.project_root.rglob('*')):
        if not p.is_file(): continue
        if '.venv' in p.parts or '__pycache__' in p.parts: continue
        rel = p.relative_to(orchestrator.project_root).as_posix()
        if any(rel.startswith(s.lstrip('.')) or p.name.startswith('.boom3') for s in skip): continue
        ext = p.suffix.lower()
        files.append({
            "path": rel,
            "language": exts.get(ext, 'text'),
            "agent": "generated",
            "exports": [],
        })
...
def get_file_content(project_id, filepath):
    ...
    if not orchestrator:
        return jsonify({"error": "Project not found"}), 404
    content = orchestrator.file_manager.read(filepath)
    if content is None:

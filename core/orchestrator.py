  ...
  "changelog": ["Blueprint created by Architect"]
    ...
    def _build_module(self, module: Dict, blueprint: Blueprint) -> bool:
        ...
        for attempt in range(1, max_build_attempts + 1):
            # Keep system prompt lean — only what this module needs
            # (full blueprint in every message causes 431 header-too-large)
            system = f"""You are a Builder agent writing {filepath}.

PURPOSE: {purpose}
STACK: {blueprint.stack}
APP: {blueprint.name} — {blueprint.purpose[:200]}

RULES:
{chr(10).join(f'- {r}' for r in blueprint.rules[:8])}
...
WIRING CONTEXT:
{chr(10).join(f'  {w["from"]} -> {w["to"]}' for w in blueprint.wiring[:10])}
...
Return ONLY the raw file content. No markdown fences. No explanation."""
            ...
            # Keep history short — only last 2 exchanges to avoid 431 errors
            trimmed_history = history[-4:] if len(history) > 4 else history
            messages = [{"role": "system", "content": system}] + trimmed_history + [{"role": "user", "content": user_msg}]
    ...
    def _validate_module(self, filepath: str, code: str, module: Dict, blueprint: Blueprint) -> List[str]:
        """
        Strict validator — rejects stubs, empty functions, and placeholder comments.
        A file only passes if it has real working implementation.
        """
        errors = []
        if not code or len(code.strip()) < 50:
            return ["File is empty or too short"]

        # ── Stub detection — reject files that are mostly placeholders ────────
        stub_patterns = [
            (r"//\s*(implementation|TODO|FIXME|placeholder|add.*here)",  "stub comment"),
            (r"#\s*(TODO|FIXME|placeholder|implementation here|add.*here)", "stub comment"),
        ]
        stub_hits = 0
        for pattern, label in stub_patterns:
            stub_hits += len(re.findall(pattern, code, re.IGNORECASE))
        if stub_hits >= 2:
            errors.append(f"STUB DETECTED: {stub_hits} placeholder comments — write real implementation")

        # Count real logic indicators
        logic_count = len(re.findall(
            r"(if|for|while|return|SELECT|INSERT|CREATE TABLE|addEventListener|fetch\(|querySelector)",
            code, re.IGNORECASE
        ))
        if logic_count < 2 and len(code) < 300:
            errors.append(f"INSUFFICIENT CODE: only {logic_count} logic statements — looks like a stub")
        ...
        if filepath.endswith(".py"):
            ...
            for export in exports:
                if export and not re.search(rf"def {export}|class {export}|{export}\s*=", code):
            ...
            tables = blueprint.schema.get("tables", [])
            if tables and module.get("role") in ("database_manager", "backend_logic"):
                table_name = tables[0]["name"]
                if table_name not in code:
                    errors.append(f"Table '{table_name}' not referenced in code")

        elif filepath.endswith(".html"):
            if "<html" not in code.lower() and "<!doctype" not in code.lower():
                errors.append("HTML missing <html> tag")
            if len(code) < 300:
                errors.append("HTML too short to be a real UI")

        elif filepath.endswith(".js"):
            if logic_count < 2:
                errors.append("JS file has no real logic — likely a stub")
...
CURRENT FILES:
{self.file_manager.all_python_files()[:12000]}
...
Return the JSON patch now."""
            ...
            # Trim to last 4 messages to avoid 431 header errors
            trimmed_repair = repair_history[-4:] if len(repair_history) > 4 else repair_history
    ...
    def _review(self, contract: ProjectContract, blueprint: Blueprint) -> Dict:
        """
        Honest reviewer. Checks each feature has WORKING CODE behind it.
        A filename or stub function does NOT count as a feature being present.
        """
        ...
        for p in self.project_root.rglob("*"):
            ...
            if p.name.startswith(".boom3"): continue
            try:
                files[p.relative_to(self.project_root).as_posix()] =                     p.read_text(encoding="utf-8", errors="replace")
            except Exception:
                pass
        ...
        # Count stubs in entire codebase
        stub_count = len(re.findall(
            r'(//\s*(implementation|todo|placeholder|stub|add here)'
            r'|#\s*(todo|placeholder|stub|implementation here)'
            r'|pass\s*$'
            r'|raise\s+notimplementederror)',
            all_code, re.MULTILINE | re.IGNORECASE
        ))

        def has_real_impl(signals, min_matches=2):
            matched = [s for s in signals if s in all_code]
            if len(matched) < min_matches:
                return False
            # Reject if there are more stubs than signal matches
            return stub_count < (len(matched) * 3)

        # Feature map — signals must be EXECUTABLE CODE not just names
        feature_checks = {}

        if any(w in desc for w in ["file explorer", "file tree"]):
            feature_checks["file explorer"] = {
                "signals": ["/api/files", "os.listdir", "os.walk", "readdirSync",
                            "file-tree", "filetree", "innerhtml +=", "createelement"],
                "min": 2, "description": "API that reads files + UI that renders them"
            }
        if "code editor" in desc:
            feature_checks["code editor"] = {
                "signals": ["codemirror", "monaco.editor", "ace.edit", "textarea",
                            "/api/file", "editor.getvalue", "editor.setvalue"],
                "min": 2, "description": "Real editor component with file load/save"
            }
        if any(w in desc for w in ["real-time log", "live log", "logs"]):
            feature_checks["real-time logs"] = {
                "signals": ["eventsource", "setinterval", "/api/logs", "appendchild",
                            "textcontent +=", "scrolltop", "websocket"],
                "min": 2, "description": "Polling or SSE + UI that appends log lines"
            }
        if "test runner" in desc:
            feature_checks["test runner"] = {
                "signals": ["subprocess.run", "subprocess.popen", "pytest",
                            "/api/test", "/api/run-tests", "run_tests"],
                "min": 2, "description": "Subprocess runs pytest + API + UI button"
            }
        if any(w in desc for w in ["auto-repair", "auto repair"]):
            feature_checks["auto-repair"] = {
                "signals": ["openai", "chat.completions", "ai_client",
                            "/api/repair", "auto_repair", "repair_loop"],
                "min": 2, "description": "AI repair loop that patches files"
            }
        if "git" in desc:
            feature_checks["git integration"] = {
                "signals": ["subprocess.run.*git", "import git", "from git",
                            "git init", "git commit", "/api/git", "gitpython"],
                "min": 2, "description": "Real git commands via subprocess or GitPython"
            }
        if any(w in desc for w in ["deploy", "render"]):
            feature_checks["deploy to render"] = {
                "signals": ["render.yaml", "render.com", "/api/deploy",
                            "fetch.*deploy", "gunicorn"],
                "min": 2, "description": "render.yaml + deploy API endpoint"
            }
        if any(w in desc for w in ["dependency", "install"]):
            feature_checks["dependency installer"] = {
                "signals": ["subprocess.*pip", "pip install", "/api/install",
                            "install_package", "requirements.txt"],
                "min": 2, "description": "subprocess pip install + API endpoint"
            }
        if "docker" in desc:
            feature_checks["docker"] = {
                "signals": ["docker.from_env", "import docker", "client.containers",
                            "container.run", "dockerfile"],
                "min": 2, "description": "Docker SDK to create/run containers"
            }
        if any(w in desc for w in ["multi-agent", "multi agent"]):
            feature_checks["multi-agent"] = {
                "signals": ["threading.thread", "asyncio", "queue.queue",
                            "agent_role", "run_agent", "task_queue"],
                "min": 2, "description": "Multiple agents running concurrently"
            }

        present, missing = [], []
        for feature, cfg in feature_checks.items():
            if has_real_impl(cfg["signals"], cfg["min"]):
                present.append(feature)
                self.add_log(f"    ✅ {feature}: real implementation found")
            else:
                missing.append(feature)
                self.add_log(f"    ❌ {feature}: NOT implemented — needs: {cfg['description']}")

        total = len(present) + len(missing)
        score = int(len(present) / total * 100) if total > 0 else 0
        passed = score >= 60

        self.add_log(f"    Honest score: {score}/100 ({len(present)}/{total} features real, {stub_count} stubs found)")

        return {
            "passed": passed, "score": score,
            "missing": missing, "present": present,
            "summary": f"{score}/100 — {len(present)} real, {len(missing)} missing"
        }
